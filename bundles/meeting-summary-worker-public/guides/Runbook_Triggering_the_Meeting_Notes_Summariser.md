...
Use this when:
- the source meeting material has already been uploaded to the source folder
- you want the assistant to run the house-style meeting-summary workflow
- you want a simple, repeatable command pattern across chat surface, Discord, and other chat surfaces

---

## What the assistant assumes by default
...
- the house-style meeting-summary workflow should be used
...
## Source
Upload the input file to the source folder.
...
- the final summary
- the assistant’s rubric-based self-check
...
# What the assistant will infer automatically
...
3. the assistant processes them in sequence

---

# Current limitation to understand

Right now, the specialist worker is not fully persistent on this chat surface surface because thread-bound persistent worker support is blocked by surface/platform constraints.
...
If you want a true blind evaluation, do not upload the handwritten target until after the assistant has produced the first draft.
...
1. upload source material to source folder
2. optionally upload matching negative / target files
3. trigger the assistant with a standard command like:
