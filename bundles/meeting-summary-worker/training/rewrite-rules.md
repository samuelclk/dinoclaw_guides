# Rewrite Rules: Raw / Gemini Notes -> Samuel-Style Summary
...
Convert from:
- “Samuel explained how X works using A, B, and C”

Into:
- “Presented Lido Earn / DI integration model”
...
### 7. Write notes in Samuel’s preferred density
...
Examples:
- `Mobee already utilizes Lido Finance to deploy their Ethereum users' assets for their "simple earn" product.`
- `Hashkey Exchange and OTC do not support margin trading yet.`
...
- would Samuel actually track this later?
...
## Explicit Gemini -> target insights from examples 1-3
...
If the meeting's value lies in Samuel redirecting the conversation from one path to a better one, preserve that pivot explicitly.

Example pattern:
- Gemini: broad recap of company background + explored ideas
- Samuel-style target: `Explained why the original ask should go elsewhere, then redirected to 5 angles to explore`

### Prefer dependency logic over meeting logistics
For GTM / partnership discussions, Samuel keeps the key dependency that determines whether outreach is worth doing.

Example pattern:
- not all event chatter
- but `this only makes sense if Kraken and Lido has an active integration`
...
- Samuel-style target: `The problem seems to be two-fold...`
...
Samuel keeps them only if they materially affect progress.
...
Negative:
- long paragraphs on how Lido Earn, curators, looping, or interfaces work
...
If Samuel redirects the conversation from an unworkable path to a better one, keep that pivot.
