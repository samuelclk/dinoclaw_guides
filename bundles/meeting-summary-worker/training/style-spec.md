# Samuel-Style Meeting Summary Spec (First Pass)

Built from 10 paired examples in:
- `downloaded/source/`
- `downloaded/target/`
- `downloaded/negative/`
...
The summary should help Samuel quickly remember:
...
Good candidates:
- actual interest in Lido Earn / DI / stETH / stVaults
...
- repeated explanations of how Lido works
...
A “key discussion point” in Samuel-style output is **not** just something that was discussed.
...
- keep only the points Samuel would likely refer back to later
...
2. **Explanatory overkill**
   - multiple paragraphs explaining product mechanics Samuel already knows
3. **False importance**
...
- Samuel likes summaries that read like **internal operator notes**, not meeting minutes.
- He prefers **compressed business framing** over broad descriptive prose.
...
- Strong confidence on **rewrite style** from all 10 examples.
- After the explicit Gemini->target pass on examples 1-3, the rewrite signal is stronger because those first 3 examples now contribute to rewrite learning as well, not just transcript extraction contrast.
